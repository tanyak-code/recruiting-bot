#!/usr/bin/env node
/**
 * SWE II Frontend Screener — Brainner → Claude → HTML/CSV
 * Pulls all Application Review candidates from Brainner for Andre's SWE II role,
 * screens with criteria, generates HTML report + CSV.
 *
 * Run: node screen-andre-mid.js
 */
...
const JOB_SLUG      = "fdc5a393-a00a-43b2-ac8c-048211dfebec"; // SWE II Frontend (Andre)
const TOP_N         = 1000; // Top 1000 by Brainner score
const STOP_AT_GOOD  = 50;   // Stop once 50 score 3+ candidates found (cached + fresh)
const DRY_RUN       = process.argv.includes("--dry-run");
const HTML_FILE  = path.join(__dirname, "andre-mid-report.html");
const CSV_FILE   = path.join(__dirname, `andre-mid-screened-${new Date().toISOString().slice(0,10)}.csv`);
const CACHE_FILE = path.join(__dirname, "andre-mid-cache.json");

const { criteria: ANDRE_CRITERIA } = require("./criteria/andre-mid");
...
<head>
  <meta charset="UTF-8">
  ${refreshMeta}
  <title>Andre · SWE II Frontend Screener</title>
  <style>
...
<body>
  <h1>Andre · SWE II Frontend — Application Review Screener</h1>
  <div class="meta">Screening all Application Review candidates sorted by Brainner score &nbsp;·&nbsp; ${statusBadge}</div>
...
// Returns top TOP_N candidates by Brainner score — regardless of cache.
// Cache check happens in main() so all TOP_N show up in the HTML report.
async function fetchCandidates() {
  ...
  while (candidates.length < TOP_N) {
    process.stdout.write(`  Page ${page} (${candidates.length} fetched so far)… `);
  ...
  console.log(`  Fetched ${candidates.length} candidates from Brainner\n`);
...
async function screenCandidate(cand) {
  ...
  const prompt = `You are a strict senior technical recruiter screening for a mid-level frontend software engineering role. Most candidates will NOT pass — this is a specific role with hard experience requirements.
...
CALIBRATION:
- Score 4: rare — every hard requirement met with explicit evidence + growth engineering signal
- Score 3: solid frontend candidate with consumer experience, may lack growth signal
- Score 2: correct score for most candidates — wrong experience type, wrong level, or key gaps
- Score 1: immediate disqualifier present
...
Respond ONLY with a JSON object, no markdown:
{
  "name": "<full name>",
  "score": <1|2|3|4>,
  "scoreLabel": "<Strong No | No | Yes | Yes XP- | Yes XP+ | Strong Yes | Strong No* | No* | Yes* | Strong Yes*>",
  "summary": "<2-3 sentences: cite specific evidence from resume, include years of experience>",
...
// ─── Main ──────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`\n━━━ Andre · SWE II Frontend Screener ━━━`);
  console.log(`Pulling top ${TOP_N} from Brainner "To Review" sorted by score`);
  console.log(`Mode: ${DRY_RUN ? "🔍 DRY RUN" : "🚀 LIVE"}\n`);

  const cache = loadCache();
  console.log(`  Loaded cache: ${Object.keys(cache).length} previously screened\n`);

  const candidates = await fetchCandidates();

  // Split: already cached vs needs screening
  const fromCache  = [];
  const toScreen   = [];
  ...
  if (DRY_RUN) {
    ...
    console.log(`   ${toScreen.length} would be screened fresh`);
    console.log(`   ${fromCache.length} would be pulled from cache`);
    const cost = (toScreen.length * 0.004).toFixed(2);
    console.log(`   Estimated cost: ~$${cost}`);
    console.log(`\nTo run for real: node screen-andre-mid.js\n`);
    return;
  }
  ...
  for (let i = 0; i < toScreen.length; i += CONCURRENCY) {
    ...
    await Promise.all(batch.map(async (cand) => {
      try {
        ...
        console.log(`[${done}/${total}] ${emoji} ${score} — ${(result.scoreLabel||"").padEnd(14)} ${tally}  ${row.name}`);
      } catch (e) {
        done++;
    ...
    if ((counts[3] || 0) + (counts[4] || 0) >= STOP_AT_GOOD) {
      console.log(`\n🛑 Reached STOP_AT_GOOD (${STOP_AT_GOOD} score 3+ found) — stopping early`);
      break;
    }
  }

  // Write CSV for all results (cached + freshly screened)
  results.sort((a, b) => b.ourScore - a.ourScore || (b.brainnerScore||0) - (a.brainnerScore||0));
  results.forEach((r, idx) => r.rank = idx + 1);
  results.forEach((r, idx) => appendCSV(idx+1, r, r));
  writeHTML(results, total, total, false);

  console.log(`\n✅ Done — ${total} total in report (${toScreen.length} screened fresh, ${fromCache.length} from cache)`);
