#!/usr/bin/env node
/**
 * Sr. TPM — IC role (Ed) — move score 1/2 → To Be Archived & Dispo
 * ⚠️  THIS IS THE IC ROLE — NOT Sr. Manager TPM (move-srtpm-12.js)
 *
 * Ashby Job ID: 42ed66d5-dc57-4448-bd27-945b6ee91923
 * Cache:        srtpm-cache.json
 *
 * Step 1: run with DRY_RUN = true  → stage discovery + plan, no moves made
...
const JOB_ID     = "c77a45c6-9bc8-4758-a584-ee7433a45f31"; // Sr. TPM IC (Ed) — NOT Sr. Manager TPM
...
async function main() {
  ...
  console.log(`\n📋 Sr. TPM IC (Ed) — score 1/2 → To Be Archived & Dispo`);
  console.log(`   ⚠️  NOT Sr. Manager TPM — IC role, job ${JOB_ID}`);
  ...
  // ── Discover stage IDs from application objects ─────────────────────────────
  ...
  console.log(`\n   All stages found in applications (add to CLAUDE.md after dry run):`);
  ...
  if (DRY_RUN) {
    console.log(`\n👆 Dry run complete. Verify the stage IDs above match CLAUDE.md, then set DRY_RUN = false and rerun.`);
  }
}
