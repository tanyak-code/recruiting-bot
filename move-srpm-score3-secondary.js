#!/usr/bin/env node
/**
 * Move score-3 Sr. Product Manager candidates from Application Review → Secondary Review
 * Reads sr-pm-cache.json, finds all score-3 candidates,
 * matches by email in Ashby, and ONLY moves them if they are
 * currently in "Application Review". Skips anyone in any other stage.
 *
 * Run: node move-srpm-score3-secondary.js
 */
...
async function main() {
  ...
  for (const app of applications) {
    ...
    if (!secondaryReviewStageId && title.includes("secondary review")) secondaryReviewStageId = stage.id;
  ...
  if (!appReviewStageId || !secondaryReviewStageId) {
  ...
  console.log(`   ✅ Secondary Review:    ${secondaryReviewStageId}`);
