#!/usr/bin/env node
/**
 * Archive score-1 and score-2 Sr. Manager TPM candidates in Ashby
 * Reads sr-tpm-cache.json, finds all score-1 and score-2 candidates,
...
const JOB_ID           = "59d95ceb-ba5e-4c01-8c05-c99076529012"; // Sr. Manager TPM
...
async function main() {
  ...
  console.log(`\n📥 Fetching all applications for Sr. Manager TPM job...`);
  const applications = await fetchAllApplications(JOB_ID);
  console.log(`   Found ${applications.length} active applications`);

  // Discover Application Review stage ID from actual applications
